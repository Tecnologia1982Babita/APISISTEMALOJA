...
const rows = await db.query('select coalesce((((((venda-meta)*-1)*100)/((case when((meta) = 0) then 1 else ((meta)) end)))::numeric(10,2)),0) as procentagemfalta, coalesce(((((venda)*100)/((case when((meta) = 0) then 1 else ((meta)) end)))::numeric(10,2)),0) as porcentagemvenda,((venda-meta)*-1) as falta,\r\
* from (select rev_nom, produtos_revista,codigovend,ven_nome,sum(venda) as venda, sum(meta) as meta from (\r\
select * from (\r\
select \r\
metven_datini,\r\
...
(loj_cod ='+novaloja+') group by case when((rev_cod) > 2) then '+colecao+' else case when(rev_cod = 1) then '+revistaregular+' else '+revistaplus+' \r\
end end,coalesce(erp_vendedores.ven_nome,meta_vendas_total.ven_nome),metven_valdiaria,metven_datini,rev_cod,erp_meta_vendedora.ven_cod)a)b \r\
group by rev_nom, produtos_revista,codigovend,ven_nome order by ven_nome,produtos_revista)a');
